...
const conversationComparisonByDate = (conv1, conv2) => {
	const v1 = new Date(conv1.messages[conv1.messages.length - 1].date)
	const v2 = new Date(conv2.messages[conv2.messages.length - 1].date)
	return v1 < v2 ? 1 : -1
}
const conversationComparisonByPinDate = (conv1, conv2) => {
	const v1 = new Date(conv1.pinChangeDate)
	const v2 = new Date(conv2.pinChangeDate)
	return v1 > v2 ? 1 : -1
}
